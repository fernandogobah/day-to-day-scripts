...
deb-src http://repo.mysql.com/apt/debian/ ${codename} mysql-5.7" > /etc/apt/sources.list.d/mysql.list
        ...
        apt-get install -y bacula-mysql bacula-aligned
        ...
        yum install -y bacula-mysql bacula-aligned
    fi
...
function install_with_postgresql()
{
    if [ "$OS" == "debian" -o "$OS" == "ubuntu" ]; then
        ...
        apt-get install -y bacula-postgresql bacula-aligned

    elif [ "$OS" == "centos" ]; then
        yum install -y postgresql-server
        yum install -y bacula-postgresql bacula-aligned --exclude=bacula-mysql
